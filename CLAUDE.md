...
- `src/qr_policy/` — the HPN policy adapter (`hpn_policy_module.py`); `src/qr_main/` (perceive-plan-act loop), `src/qr_run/` (assembly: `run.run_QR_main(problem, config)`, `virtual_robots.py` QDDL→scene bridges for both MuJoCo and kinsim backends), `src/qr_problem_defs/` + `src/qr_domains/` (problems and QDDL files). The combined run defaults to the kinematics-only simulator (`Ruby_Kinsim`; cameras via its MuJoCo render bridge): `pixi run combined`, or `--virtual-robot Ruby_Mujoco_Sim` for dynamics. Run a problem: build a script like `run_QR_main(ruby_1_move_base, QRSystemConfig(segmentation_method="sim", hpn_params=HPNConfig(debug_level=1, interactive=False)))` — `interactive=False` is required headless (HPN's debug tags pause on input() otherwise). HPN planning logs land in `~/QR_logs`/`~/HPN_logs`; the old repo's pkl recordings there can be replayed via `QRSystemConfig(run_from_pkl_path=...)`.
- KNOWN ISSUES (HPN-side, for the "upgrade HPN" phase — not NQR bugs):
  - `ruby_1_move_base` (2026-06-10): the first plan from an empty belief fails (`plan_fail: monotonic`, no look_region generated). The unmodified old repo fails identically against today's HPN; 2026-05-31 logs in `~/QR_logs` show it working then. Per tlp, this problem "does not currently work" — use `ruby_pick_place_move_base`.
  - `ruby_pick_place_move_base` (2026-06-11): the combined system runs the live loop correctly — 10 planned actions executed (5 look_region, 5 move), all objects perceived, grasps generated — then crashes in HPN's `CogMan/plan_defs.py:371` (`found_sticky_sat_preimage`: `self.steps[i + 1][0]` IndexError when the satisfied preimage is the plan's last step), and the process hangs in shutdown.
